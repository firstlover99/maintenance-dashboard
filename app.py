...
import re
import requests
import os, hashlib, functools, pickle
...
def _calc_cache_dir():
    return os.path.join(_app_dir(), '_calc_cache')

def _df_signature(df, *params):
    try:
        n = len(df)
        s = float(pd.to_numeric(df['소요시간'], errors='coerce').fillna(0).sum()) if '소요시간' in df.columns else 0.0
        mx = str(df['발생일시'].max()) if '발생일시' in df.columns else ''
    except Exception:
        n, s, mx = (len(df) if hasattr(df, '__len__') else 0), 0.0, ''
    raw = f'{n}|{s:.1f}|{mx}|{params!r}'
    return hashlib.md5(raw.encode('utf-8')).hexdigest()[:16]

def _disk_memo(kind, keep=16, key_filter=None):
    """첫 인자(df) 시그니처+나머지 인자로 디스크 캐시. 앱 재시작에도 결과 보존.
    key_filter: 시그니처 계산 전 df를 정규화(예: 소요시간 notna)해 동일결과 호출간 캐시 공유."""
    def deco(fn):
        @functools.wraps(fn)
        def wrap(df, *args, **kwargs):
            p = None
            try:
                kdf = key_filter(df) if (key_filter is not None) else df
                sig = _df_signature(kdf, kind, args, tuple(sorted(kwargs.items())))
                p = os.path.join(_calc_cache_dir(), f'{kind}_{sig}.pkl')
                if os.path.exists(p):
                    try:
                        with open(p, 'rb') as f:
...
def _count_clusters(g, cluster_min=60):
    """cluster_faults와 동일 규칙으로 클러스터 '개수'만 빠르게 계산 (numpy, iterrows 제거).
    calc_mttr_mtbf는 개수와 소요시간합만 쓰는데, 소요합은 클러스터링과 무관하게 보존됨."""
    g = g.sort_values('발생일시')
    occ = g['발생일시'].to_numpy()
    end = g['완료시각'].to_numpy() if '완료시각' in g.columns else np.full(len(g), np.datetime64('NaT'))
    start = g['출동시각'].to_numpy() if '출동시각' in g.columns else np.full(len(g), np.datetime64('NaT'))
    n = len(g)
    if n == 0:
        return 0
    count = 1
    ...
    for i in range(1, n):
        ...
        if pd.isna(pe):
            pe = cur_start
        gap = None
        if not pd.isna(ns_v) and not pd.isna(pe):
            gap = (ns_v - pe) / np.timedelta64(1, 'm')
        if gap is not None and 0 <= gap <= cluster_min:
            if not pd.isna(end[i]):
                cur_end = end[i]
        else:
            count += 1
            cur_end = end[i]
...
def load_press(file_obj=None, raw_df=None):
    if raw_df is not None:
        df = raw_df
    else:
        xf = pd.ExcelFile(file_obj)
        if '설비보전현황_통합' in xf.sheet_names:
            sheet = '설비보전현황_통합'
        elif 'Sheet1' in xf.sheet_names:
            _t = pd.read_excel(file_obj, sheet_name='Sheet1', nrows=1); file_obj.seek(0)
            sheet = 'Sheet1' if ('년' in _t.columns and '설비유형' in _t.columns) else xf.sheet_names[0]
        else:
            sheet = xf.sheet_names[0]
        file_obj.seek(0)
        df = pd.read_excel(file_obj, sheet_name=sheet, header=0)
    df = df.dropna(how='all').reset_index(drop=True)  # 빈 행 제거 (SP3K 등 100만행 방지)
    _raw = df.columns.tolist()
    # SP3K 감지: col[3]=차종 OR 출동시간/소요시각 컬럼 존재 (표준은 출동시각/소요시간)
    _sp3k = (len(_raw) == 17 and
             (str(_raw[3]).strip() == '차종' or
              any(str(c).strip() in ('출동시간', '출동시간 ', '소요시각') for c in _raw)))
    if _sp3k:
        # SP3K: col15=비고, col16=조치자 (표준과 반대)
        df.columns = ['년','월','일','_차종_raw','라인','정지시각','출동시각','완료시각',
                      '소요시간','설비유형','고장설비','고장부위','현상','원인','조치내역',
                      str(_raw[15]).strip(), str(_raw[16]).strip()]
        # 비고↔조치자 표준 컬럼명으로 보정
        if '비고' in df.columns and '조치자' not in df.columns:
            df.rename(columns={'비고': '조치자'}, inplace=True)
        if str(_raw[16]).strip() == '조치자' and str(_raw[15]).strip() == '비고':
            pass  # 이미 올바름
        elif str(_raw[15]).strip() == '조치자' and str(_raw[16]).strip() == '비고':
            pass  # 표준과 동일
        else:
            df.rename(columns={str(_raw[15]).strip(): '조치자',
                               str(_raw[16]).strip(): '비고'}, inplace=True)
    else:
    ...
    df['파일출처'] = '프레스'
    if '_차종_raw' in df.columns:
        df['차종'] = df['_차종_raw'].apply(
            lambda v: str(v).strip() if pd.notna(v) and str(v).strip() not in ('', 'nan', 'None') else None)
        df.drop(columns=['_차종_raw'], inplace=True)
    else:
        df['차종'] = None
    df['조치'] = df['조치내역']
...
def load_robot(file_obj=None, raw_df=None):
    if raw_df is not None:
        df = raw_df
    else:
        file_obj.seek(0)
    ...
    # 컬럼 수 불일치 시 press 포맷으로 fallback (SP3K 등 17컬럼 파일)
    if len(df.columns) != 21:
        if raw_df is not None:
            return load_press(raw_df=df)
        file_obj.seek(0)
        return load_press(file_obj)
...
def detect_and_load(file_obj, fname=''):
    try:
        ...
        if any(c in cols for c in ['라인_KEY','고장부위_STD']) and len(cols) >= 20:  # SP3K는 17컬럼이므로 제외
...
def merge_dfs(press_df, robot_df):
    ...
    # ── 최근 3년만 유지 (속도·관심기간). 현재연도 기준, 미래/과거 오타연도 제외 ──
    _now_y = datetime.now().year
    merged = merged[(merged['년'] >= _now_y - 2) & (merged['년'] <= _now_y)].copy()
    if merged.empty:
        return None
...
def _fetch_gsheet_csv_df(url):
    """구글시트를 CSV로 빠르게 받아 DataFrame 반환 (기본 시트). xlsx보다 파싱 빠름."""
    file_id = None
    for p in [r'spreadsheets/d/([a-zA-Z0-9_-]+)', r'/d/([a-zA-Z0-9_-]+)', r'id=([a-zA-Z0-9_-]+)']:
        m = re.search(p, url)
        if m: file_id = m.group(1); break
    if not file_id: raise ValueError('파일 ID를 찾을 수 없습니다.')
    ...
    if len(resp.content) < 100: raise ValueError('CSV가 너무 작습니다 — 공유 권한 확인.')
    return pd.read_csv(io.BytesIO(resp.content))

def _load_gsheet_fast(url):
    """CSV로 읽어 컬럼 기준 press/robot 라우팅. (df, ftype) 반환."""
    raw = _fetch_gsheet_csv_df(url)
    cols = raw.columns.tolist()
    if any(c in cols for c in ['라인_KEY', '고장부위_STD']) and len(cols) >= 20:
        return load_robot(raw_df=raw), 'robot'
    if all(c in cols for c in ['년', '월', '라인', '설비유형']):
        return load_press(raw_df=raw), 'press'
    if len(cols) >= 20:
        return load_robot(raw_df=raw), 'robot'
    if len(cols) >= 15:
        return load_press(raw_df=raw), 'press'
    return None, f'인식불가({len(cols)}열)'
...
@_disk_memo('surge')
def _detect_surge_cached(d):
    """급증 설비 감지 (최근 7일 ≥3건 & 직전 21일 주평균 2배↑). 캐시."""
    if d is None or d.empty or '발생일시' not in d.columns:
        return pd.DataFrame()
    ref = d['발생일시'].max()
    rows = []
    last7_start = ref - timedelta(days=7)
    prior_start = ref - timedelta(days=28)
    for key, g in d.groupby('설비_KEY'):
        last7 = int((g['발생일시'] > last7_start).sum())
        prior = int(((g['발생일시'] > prior_start) & (g['발생일시'] <= last7_start)).sum())
        prior_wk = prior / 3.0
        if last7 >= 3 and (prior_wk == 0 or last7 >= 2 * prior_wk):
            rows.append({'설비': key, '최근7일': last7, '평소 주평균': round(prior_wk, 1),
                         '배수': round(last7 / prior_wk, 1) if prior_wk > 0 else float('inf')})
    if not rows:
        return pd.DataFrame()
    return pd.DataFrame(rows).sort_values(['최근7일', '배수'], ascending=False).reset_index(drop=True)

@st.cache_data(ttl=3600, max_entries=16, show_spinner=False)
@_disk_memo('mttr', key_filter=lambda d: d[d['소요시간'].notna()] if '소요시간' in getattr(d,'columns',[]) else d)
def calc_mttr_mtbf(df, cluster_min=60):
    ...
    for key, grp in df2.groupby('설비_KEY'):
        ...
        if len(bm_grp) >= 2:
            # 클러스터 개수만 빠르게 (소요시간 합은 클러스터링과 무관하게 보존)
            cluster_cnt = _count_clusters(bm_grp, cluster_min=cluster_min)
...
@st.cache_data(ttl=3600, max_entries=16, show_spinner=False)
@_disk_memo('gap')
def _gap_analysis(rdf):
    """동일 설비·부위 재발 간격 분석 (TAB11). 무거운 그룹 루프 → 캐시."""
    rdf = rdf.copy()
    ...
    if not gap_rows:
        return pd.DataFrame()
    return pd.DataFrame(gap_rows).sort_values('평균재발간격_일')

@st.cache_data(ttl=3600, max_entries=16, show_spinner=False)
@_disk_memo('parts')
def _parts_cycle(sdf):
    """부품교체 주기 추정 (TAB12). 무거운 그룹 루프 → 캐시."""
    if '조치코드' not in sdf.columns:
        return pd.DataFrame()
    ...
    if not parts_rows:
        return pd.DataFrame()
    return pd.DataFrame(parts_rows).sort_values('평균교체주기_일')
...
# ── 사전 등록 구글시트 (앱 진입 시 자동 로드) ──────────
REGISTERED_SHEETS = [
    ('차체',   'https://docs.google.com/spreadsheets/d/1cPL_uZW0m4a1AWYqbA6zNunxEIHdaOUAQQXHyP0oSNw/edit'),
    ('프레스', 'https://docs.google.com/spreadsheets/d/1DTRqt9X_Ut7QWA6-rSAfyqEwis8KqtckQtWJl3FLhsQ/edit'),
]

# ── 로컬 스냅샷 캐시 (업무일 신선도) ──────────────────
import os, pickle as _pickle
SNAPSHOT_SCHEMA = 1     # merged 스키마 변경 시 +1 (자동 무효화)
UPDATE_HOUR = 9         # 평일 데이터 갱신 완료 기준 시각 (보통 08~09시)
...
def _snapshot_path():
    try:
        ...
        base = os.getcwd()
    return os.path.join(base, '_data_snapshot.pkl')

def _last_business_update(now):
    """가장 최근의 평일 UPDATE_HOUR:00 시각. 주말은 직전 평일로."""
    ref = now.replace(hour=UPDATE_HOUR, minute=0, second=0, microsecond=0)
    if now < ref:
        ref = ref - timedelta(days=1)
    while ref.weekday() >= 5:   # 토(5)·일(6) 건너뜀
        ref = ref - timedelta(days=1)
    return ref

def _snapshot_load(ignore_fresh=False):
    """스냅샷 dict 반환(없으면 None). ignore_fresh=True면 신선도 무시(있으면 반환)."""
    p = _snapshot_path()
    ...
    try:
        ...
        if ignore_fresh:
            return snap
        _now = datetime.now()
        # 업무일 기준 또는 20시간 이내면 신선 (클라우드 커밋 스냅샷 채택용)
        if ts < _last_business_update(_now) and (_now - ts) > timedelta(hours=20):
            return None
        return snap
    except Exception:
        return None

def _snapshot_save(press, robot, merged):
    try:
        with open(_snapshot_path(), 'wb') as f:
            _pickle.dump({'schema': SNAPSHOT_SCHEMA, 'ts': datetime.now(),
                          'press': press, 'robot': robot, 'merged': merged}, f)
        return True
    except Exception:
        return False

def _restore_from_bundle():
    """클라우드용: 커밋된 _cloud_warm.pkl.gz가 있으면 스냅샷/캐시로 1회 복원 (재계산·재fetch 회피)."""
    snap_p = _snapshot_path()
    bundle = os.path.join(_app_dir(), '_cloud_warm.pkl.gz')
    if os.path.exists(snap_p) or not os.path.exists(bundle):
        return
    try:
        import gzip as _gz
        with _gz.open(bundle, 'rb') as f:
            data = _pickle.load(f)
        with open(snap_p, 'wb') as f:
            _pickle.dump(data['snapshot'], f)
        cdir = _calc_cache_dir()
        os.makedirs(cdir, exist_ok=True)
        for fname, b in (data.get('cache') or {}).items():
            try:
                with open(os.path.join(cdir, fname), 'wb') as cf:
                    cf.write(b)
            except Exception:
                pass
    except Exception:
...
def _cached_load_all(urls_key):
    """등록 시트 fetch+파싱+통합을 30분 캐시. urls_key=캐시 구분용 튜플."""
    msgs = []; press = None; robot = None
    for label, url in REGISTERED_SHEETS:
        try:
            try:
                result, ftype = _load_gsheet_fast(url)   # CSV 빠른 경로 (xlsx 대비 ~2배)
            except Exception:
                fobj, err = load_from_gdrive(url)          # 실패 시 xlsx 폴백
                if err:
                    msgs.append(f'⚠️ {label}: {err}')
                    continue
                result, ftype = detect_and_load(fobj)
            if ftype == 'press':
                press = result; msgs.append(f'✅ 프레스({label}) — {len(result):,}건')
            elif ftype == 'robot':
                robot = result; msgs.append(f'✅ 로봇/지그({label}) — {len(result):,}건')
            else:
                msgs.append(f'⚠️ {label}: 형식 인식 실패 ({ftype})')
        except Exception as e:
            msgs.append(f'⚠️ {label}: {e}')
    merged = merge_dfs(press, robot)
    return press, robot, merged, msgs

def _apply_default_filter(merged):
    """기본 필터: 올해 1월~당월(월선택). 올해 데이터 없으면 최신연도 전체."""
    _now = datetime.now()
    _yrs = sorted(merged['년'].dropna().unique().astype(int)) if '년' in merged.columns else []
    _yr = _now.year if _now.year in _yrs else (_yrs[-1] if _yrs else _now.year)
    _months = list(range(1, _now.month + 1)) if _yr == _now.year else list(range(1, 13))
    st.session_state['gf_mode'] = '📆 월선택'
    st.session_state['gf_year_single'] = _yr
    st.session_state['gf_months'] = _months
...
def _load_registered_sheets(force=False, snapshot_only=False):
    ...
    if not force:
        _restore_from_bundle()   # 클라우드: 커밋된 번들이 있으면 복원
    snap = None if force else _snapshot_load(ignore_fresh=snapshot_only)
    if snap is not None:
        press, robot, merged = snap['press'], snap['robot'], snap['merged']
        _ts = snap['ts']
        msgs = [f"⚡ 저장된 데이터 사용 — {_ts.strftime('%m/%d %H:%M')} 기준"]
        st.session_state['_data_source'] = 'snapshot'
    elif snapshot_only:
        # 진입 시 저장 데이터 없음 → 구글 자동 fetch 안 함 (갱신 버튼으로)
        return ["⏸ 데이터 미로드 — '🔄 데이터 갱신'을 눌러 구글에서 불러오세요."]
    else:
        press, robot, merged, msgs = _cached_load_all(tuple(u for _, u in REGISTERED_SHEETS))
        if merged is None:
            _cached_load_all.clear()  # 실패는 캐시하지 않음 → 자가복구
            _ts = None
        else:
            _snapshot_save(press, robot, merged)
            _ts = datetime.now()
            st.session_state['_data_source'] = 'google'
            msgs = msgs + [f"🌐 구글에서 갱신 후 스냅샷 저장 — {_ts.strftime('%m/%d %H:%M')}"]
    st.session_state.press_df = press
    st.session_state.robot_df = robot
    if merged is not None:
        st.session_state.merged_df = merged.copy()  # 캐시 원본 보호
        st.session_state['_snap_ts'] = _ts
        _apply_default_filter(merged)  # 기본: 올해 1월~당월
    return msgs

# 앱 진입 시 자동 로드 — 데이터가 없으면 자동 시도(성공 시 멈춤, 일시오류 자가복구)
# 진입 시 자동 로드 안 함 — TAB1의 '데이터 불러오기' 버튼을 눌러야 로드됨
...
st.markdown("""<style>
/* 네비 라디오를 탭처럼 — 가로 줄바꿈 + 알약 버튼 */
div[data-testid="stRadio"][aria-label="_nav"] > div{flex-direction:row;flex-wrap:wrap;gap:4px;}
div[data-testid="stRadio"][aria-label="_nav"] label{background:#eef2f8;border-radius:8px;
  padding:5px 12px;margin:0;font-size:13px;font-weight:600;border:1px solid #dde4ee;}
div[data-testid="stRadio"][aria-label="_nav"] label:hover{background:#dde8f7;}
</style>""", unsafe_allow_html=True)
_page = st.radio("_nav", _PAGES, horizontal=True, label_visibility="collapsed", key="_nav_page")
...
if _page == "📂 데이터 불러오기":
    ...
    if st.session_state.get("_upload_active"):
        st.markdown('<div class="warn-box">📤 <b>업로드 파일 모드</b> — 구글 데이터 제외, 업로드 파일만 분석 중. '
                    '아래 <b>🌐 구글 데이터로 복귀</b> 로 되돌릴 수 있습니다.</div>', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("#### ① 구글 드라이브 (기본 등록 시트)")
        _amsgs = st.session_state.get("_auto_load_msgs", [])
        if _amsgs:
            st.markdown(
                '<div class="ok-box">' + "<br>".join(_amsgs) + '</div>',
                unsafe_allow_html=True)
        elif st.session_state.merged_df is None:
            st.markdown('<div class="warn-box">📥 아래 <b>데이터 불러오기</b> 버튼을 눌러 '
                        '데이터를 불러오세요. (진입 시 자동 로드 안 함)</div>', unsafe_allow_html=True)
        _bl1, _bl2 = st.columns(2)
        with _bl1:
            if st.button("📥 데이터 불러오기", type="primary", use_container_width=True):
                with st.spinner("불러오는 중..."):
                    st.session_state["_auto_load_msgs"] = _load_registered_sheets()
                    st.session_state["_just_loaded"] = True
                st.rerun()
        with _bl2:
            if st.button("🔄 구글에서 최신 갱신", use_container_width=True):
                with st.spinner("구글에서 갱신 중..."):
                    _cached_load_all.clear()
                    st.session_state["_auto_load_msgs"] = _load_registered_sheets(force=True)
                    st.session_state["_just_loaded"] = True
                st.rerun()
        st.markdown("---")
        st.markdown("###### 추가 파일 링크 (선택)")
            ...
            for uf in uploaded:
                ...
                with st.spinner(f"{uf.name} 처리 중..."):
                    try:
                        ...
                        if ftype == 'press':
                            st.session_state['_up_press'] = result
                            _loaded_ids[_fid] = 'press'
                            st.success(f"✅ 프레스 '{uf.name}' — {len(result):,}건 (업로드)")
                        elif ftype == 'robot':
                            st.session_state['_up_robot'] = result
                            _loaded_ids[_fid] = 'robot'
                            st.success(f"✅ 로봇/지그 '{uf.name}' — {len(result):,}건 (업로드)")
                        else:
                            st.warning(f"'{uf.name}' 형식 인식 실패: {ftype}")
                    except Exception as e:
                        st.error(f"'{uf.name}' 처리 오류: {e}")
            st.session_state['_loaded_file_ids'] = _loaded_ids
            if (st.session_state.get('_up_press') is not None or
                    st.session_state.get('_up_robot') is not None):
                st.info("📤 업로드 파일 준비됨 — 아래 **[🔄 데이터 통합 실행]** 을 누르면 "
                        "구글 데이터는 제외하고 **업로드 파일만** 분석합니다.")
        else:
            st.session_state['_loaded_file_ids'] = {}  # 업로더 비워지면 ID 초기화

    # 업로드 데이터 유무 — 있으면 구글 제외하고 업로드만 사용
    _has_up = (st.session_state.get('_up_press') is not None or
               st.session_state.get('_up_robot') is not None)
    _eff_press = st.session_state.get('_up_press') if _has_up else st.session_state.press_df
    _eff_robot = st.session_state.get('_up_robot') if _has_up else st.session_state.robot_df

    st.divider()

    # ── 데이터 품질 진단 (로드 직후) ──
    with st.expander("🩺 데이터 품질 진단 (로드된 파일 기준)", expanded=False):
        for label, sdf in [('프레스', _eff_press),
                           ('로봇/지그', _eff_robot)]:
    ...
    _btn_lbl = "🔄 데이터 통합 실행 (업로드 파일만)" if _has_up else "🔄 데이터 통합 실행"
    if st.button(_btn_lbl, type="primary", use_container_width=True):
        if _has_up:
            merged = merge_dfs(st.session_state.get('_up_press'), st.session_state.get('_up_robot'))
            _src = 'upload'
        else:
        ...
        if merged is not None:
            ...
            st.session_state['_data_source'] = _src
            st.session_state['_upload_active'] = (_src == 'upload')
            _apply_default_filter(merged)  # 기본: 올해 1월~당월
            st.session_state['_just_loaded']   = True
    ...
    if st.session_state.get('_upload_active') or _has_up:
        if st.button("🌐 구글 데이터로 복귀 (업로드 제외)", use_container_width=True):
            st.session_state['_up_press'] = None
            st.session_state['_up_robot'] = None
            st.session_state['_loaded_file_ids'] = {}
            st.session_state['_upload_active'] = False
            with st.spinner("구글 데이터 다시 불러오는 중..."):
                st.session_state['_auto_load_msgs'] = _load_registered_sheets(force=True)
            st.session_state['_just_loaded'] = True
            st.rerun()

    s1, s2, s3 = st.columns(3)
    _src_tag = "업로드" if _has_up else "구글"
    with s1:
        if _eff_press is not None:
            st.markdown(f'<div class="ok-box">✅ 프레스({_src_tag})<br>{len(_eff_press):,}건</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="warn-box">⬜ 프레스 미로드</div>', unsafe_allow_html=True)
    with s2:
        if _eff_robot is not None:
            st.markdown(f'<div class="ok-box">✅ 로봇/지그({_src_tag})<br>{len(_eff_robot):,}건</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="warn-box">⬜ 로봇/지그 미로드</div>', unsafe_allow_html=True)
    with s3:
...
if _page == "📊 고장현황 (Pareto)":
    ...
    if df is None:
        ...
        def _top_cause_str(라인명):
            sub = fdf[fdf['라인_차종'] == 라인명]
            if '원인' not in sub.columns: return '-'
            col = sub['원인'].dropna().astype(str)
            col = col[col.str.strip() != '']
            if col.empty: return '-'
            top = col.value_counts().head(3)
            return '<br>'.join([f'  {i+1}. {str(k)[:30]} ({v}건)' for i,(k,v) in enumerate(top.items())]) or '-'

        def _top_action_str(라인명):
            sub = fdf[fdf['라인_차종'] == 라인명]
            if '조치내역' not in sub.columns: return '-'
            col = sub['조치내역'].dropna().astype(str)
            col = col[col.str.strip() != '']
            if col.empty: return '-'
            top = col.value_counts().head(3)
            return '<br>'.join([f'  {i+1}. {str(k)[:30]} ({v}건)' for i,(k,v) in enumerate(top.items())]) or '-'
        ...
        line_grp['_top_cause']  = line_grp['라인_차종'].apply(_top_cause_str)
        line_grp['_top_action'] = line_grp['라인_차종'].apply(_top_action_str)
        ...
        fig_ln = px.bar(line_grp, x='건수', y='라인_차종', orientation='h',
                        ...
                        custom_data=['총정지시간', '평균MTTR', '_recur', '_top_equip', '_top_faults', '_top_cause', '_top_action'])
        fig_ln.update_traces(
            hovertemplate=(
                ...
                '<b>▶ 주요 원인 Top 3</b><br>%{customdata[5]}<br>'
                '<b>▶ 주요 조치 Top 3</b><br>%{customdata[6]}<br>'
        ...
        with st.expander('📋 Top3 상세 — 라인별 전체 항목', expanded=True):
            def _s(t): return ' | '.join(p.strip() for p in t.split('<br>') if p.strip()) if t and t != '-' else '-'
            _disp1 = pd.DataFrame({
                '라인': line_grp['라인_차종'].values,
                '부위/현상 Top3': line_grp['_top_faults'].apply(_s).values,
                '원인 Top3':       line_grp['_top_cause'].apply(_s).values,
                '조치내역 Top3':   line_grp['_top_action'].apply(_s).values,
                '고장다발설비 Top3': line_grp['_top_equip'].apply(_s).values,
            })
            _disp1.index += 1
            st.dataframe(_disp1, use_container_width=True, height=min(420, (len(_disp1)+1)*35+3))
            if st.button('📥 Excel 다운로드', key='xl_ln'):
                _xl1 = _disp1.copy()
                for _c in _xl1.columns[1:]: _xl1[_c] = _xl1[_c].str.replace(' | ', '\n', regex=False)
                _xout1 = to_excel({'라인별Top3': _xl1})
                st.download_button('⬇️ 다운로드', data=_xout1,
                                   file_name=f"라인별Top3_{datetime.now().strftime('%Y%m%d')}.xlsx",
    ...
    if df is None:
        ...
        if mdf.empty:
            ...
            if mttr_df.empty:
                ...
                m_top['_ca'] = m_top['설비_KEY'].apply(lambda k: _ca_key(k, mdf))
                m_top['_ac'] = m_top['설비_KEY'].apply(lambda k: _ac_key(k, mdf))
                ...
                fig_mttr = px.bar(
                    ...
                    custom_data=['라인', '고장설비', '전체건수', 'BM건수',
                                 'MTBF_근사(시간)', '총정지시간(분)', '설비유형', '_tf', '_te', '_ca', '_ac'])
                fig_mttr.update_traces(
                    ...
                    hovertemplate=(
                        ...
                        '<b>▶ 주요 원인 Top 3</b><br>%{customdata[9]}<br>'
                        '<b>▶ 주요 조치 Top 3</b><br>%{customdata[10]}<br>'
                ...
                with st.expander('📋 Top3 상세 — MTTR 상위 전체 항목', expanded=True):
                    def _s(t): return ' | '.join(p.strip() for p in t.split('<br>') if p.strip()) if t and t != '-' else '-'
                    _disp2 = pd.DataFrame({
                        '설비': m_top['설비_KEY'].values,
                        '부위/현상 Top3': m_top['_tf'].apply(_s).values,
                        '원인 Top3':       m_top['_ca'].apply(_s).values,
                        '조치내역 Top3':   m_top['_ac'].apply(_s).values,
                        '조치자 Top3':     m_top['_te'].apply(_s).values,
                    })
                    _disp2.index += 1
                    st.dataframe(_disp2, use_container_width=True, height=min(420, (len(_disp2)+1)*35+3))
                    if st.button('📥 Excel 다운로드', key='xl_mttr'):
                        _xl2 = _disp2.copy()
                        for _c in _xl2.columns[1:]: _xl2[_c] = _xl2[_c].str.replace(' | ', '\n', regex=False)
                        _xout2 = to_excel({'MTTR_Top3': _xl2})
                        st.download_button('⬇️ 다운로드', data=_xout2,
                                           file_name=f"MTTR_Top3_{datetime.now().strftime('%Y%m%d')}.xlsx",
                                           mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                                           key='dldl_mttr')
                ...
                if not mtbf_data.empty:
                    ...
                    mtbf_data['_ca'] = mtbf_data['설비_KEY'].apply(lambda k: _ca_key(k, mdf))
                    mtbf_data['_ac'] = mtbf_data['설비_KEY'].apply(lambda k: _ac_key(k, mdf))
                    color_map_mtbf = {
                        '🔴 데이터확인필요': '#e74c3c',
                        '🟠 위험':           '#e67e22',
                        '🟢 양호':           '#27ae60',
                    ...
                    fig_mtbf = px.bar(
                        ...
                        custom_data=['라인', '고장설비', 'BM건수',
                                     '클러스터건수(BM)', 'MTTR(분)', '설비유형', 'MTBF등급', '_tf', '_te', '_ca', '_ac'])
                    ...
                    with st.expander('📋 Top3 상세 — MTBF 하위 전체 항목', expanded=True):
                        def _s(t): return ' | '.join(p.strip() for p in t.split('<br>') if p.strip()) if t and t != '-' else '-'
                        _disp3 = pd.DataFrame({
                            '설비': mtbf_data['설비_KEY'].values,
                            '부위/현상 Top3': mtbf_data['_tf'].apply(_s).values,
                            '원인 Top3':       mtbf_data['_ca'].apply(_s).values,
                            '조치내역 Top3':   mtbf_data['_ac'].apply(_s).values,
                            '조치자 Top3':     mtbf_data['_te'].apply(_s).values,
                        })
                        _disp3.index += 1
                        st.dataframe(_disp3, use_container_width=True, height=min(420, (len(_disp3)+1)*35+3))
                        if st.button('📥 Excel 다운로드', key='xl_mtbf'):
                            _xl3 = _disp3.copy()
                            for _c in _xl3.columns[1:]: _xl3[_c] = _xl3[_c].str.replace(' | ', '\n', regex=False)
                            _xout3 = to_excel({'MTBF_Top3': _xl3})
                            st.download_button('⬇️ 다운로드', data=_xout3,
                                               file_name=f"MTBF_Top3_{datetime.now().strftime('%Y%m%d')}.xlsx",
                                               mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                                               key='dldl_mtbf')
                ...
                if not scatter_d.empty:
                    ...
                    scatter_d['_ca'] = scatter_d['설비_KEY'].apply(lambda k: _ca_key(k, mdf))
                    scatter_d['_ac'] = scatter_d['설비_KEY'].apply(lambda k: _ac_key(k, mdf))
                    fig_sc = px.scatter(
                        scatter_d, x='MTBF_근사(시간)', y='MTTR(분)',
                        color='라인', size='전체건수', hover_name='설비_KEY',
                        custom_data=['라인', '고장설비', '전체건수',
                                     '총정지시간(분)', '설비유형', '_tf', '_te', '_ca', '_ac'],
                        labels={
                    ...
                    fig_sc.update_traces(
                        hovertemplate=(
                            ...
                            '<b>▶ 주요 원인 Top 3</b><br>%{customdata[7]}<br>'
                            '<b>▶ 주요 조치 Top 3</b><br>%{customdata[8]}<br>'
                    ...
                    with st.expander('📋 Top3 상세 — MTTR vs MTBF 전체 항목', expanded=False):
                        def _s(t): return ' | '.join(p.strip() for p in t.split('<br>') if p.strip()) if t and t != '-' else '-'
                        _disp4 = pd.DataFrame({
                            '설비': scatter_d['설비_KEY'].values,
                            '부위/현상 Top3': scatter_d['_tf'].apply(_s).values,
                            '원인 Top3':       scatter_d['_ca'].apply(_s).values,
                            '조치내역 Top3':   scatter_d['_ac'].apply(_s).values,
                            '조치자 Top3':     scatter_d['_te'].apply(_s).values,
                        })
                        _disp4.index += 1
                        st.dataframe(_disp4, use_container_width=True, height=min(420, (len(_disp4)+1)*35+3))
                        if st.button('📥 Excel 다운로드', key='xl_sc'):
                            _xl4 = _disp4.copy()
                            for _c in _xl4.columns[1:]: _xl4[_c] = _xl4[_c].str.replace(' | ', '\n', regex=False)
                            _xout4 = to_excel({'산점도_Top3': _xl4})
                            st.download_button('⬇️ 다운로드', data=_xout4,
                                               file_name=f"산점도_Top3_{datetime.now().strftime('%Y%m%d')}.xlsx",
                                               mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                                               key='dldl_sc')
...
if _page == "👷 인원분석":
    ...
    if df is None:
        ...
        if worker_df.empty:
            ...
            top20['_ca'] = top20['조치자'].apply(lambda w: _ca_worker(w, worker_df))
            top20['_ac'] = top20['조치자'].apply(lambda w: _ac_worker(w, worker_df))
            ...
            fig_stack.add_trace(go.Bar(
                ...
                text=top20['단독콜'],
                textposition='inside',
                customdata=top20[['단독콜','_tf','_te','_ca','_ac']].values,
                hovertemplate='<b>%{x}</b><br>단독콜: %{y}건<br><b>▶ 주요 고장유형 Top 3</b><br>%{customdata[1]}<br><b>▶ 주요 원인 Top 3</b><br>%{customdata[3]}<br><b>▶ 주요 조치 Top 3</b><br>%{customdata[4]}<br><b>▶ 담당 설비 Top 3</b><br>%{customdata[2]}<extra></extra>'))
            fig_stack.add_trace(go.Bar(
                name='협업 출동',
                ...
                text=top20['협업콜'],
                textposition='inside',
                customdata=top20[['협업콜','_tf','_te','_ca','_ac']].values,
                hovertemplate='<b>%{x}</b><br>협업콜: %{y}건<br><b>▶ 주요 고장유형 Top 3</b><br>%{customdata[1]}<br><b>▶ 주요 원인 Top 3</b><br>%{customdata[3]}<br><b>▶ 주요 조치 Top 3</b><br>%{customdata[4]}<br><b>▶ 담당 설비 Top 3</b><br>%{customdata[2]}<extra></extra>'))
            ...
            with st.expander('📋 Top3 상세 — 인원별 전체 항목', expanded=True):
                def _s(t): return ' | '.join(p.strip() for p in t.split('<br>') if p.strip()) if t and t != '-' else '-'
                _disp5 = pd.DataFrame({
                    '담당자': top20['조치자'].values,
                    '주요 고장유형 Top3': top20['_tf'].apply(_s).values,
                    '원인 Top3':           top20['_ca'].apply(_s).values,
                    '조치내역 Top3':       top20['_ac'].apply(_s).values,
                    '담당 설비 Top3':      top20['_te'].apply(_s).values,
                })
                _disp5.index += 1
                st.dataframe(_disp5, use_container_width=True, height=min(420, (len(_disp5)+1)*35+3))
                if st.button('📥 Excel 다운로드', key='xl_stk'):
                    _xl5 = _disp5.copy()
                    for _c in _xl5.columns[1:]: _xl5[_c] = _xl5[_c].str.replace(' | ', '\n', regex=False)
                    _xout5 = to_excel({'인원별Top3': _xl5})
                    st.download_button('⬇️ 다운로드', data=_xout5,
                                       file_name=f"인원별Top3_{datetime.now().strftime('%Y%m%d')}.xlsx",
                                       mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                                       key='dldl_stk')
            ...
            with pa1:
                ...
                fig_p2 = px.bar(
                    ...
                    custom_data=['출동건수', '평균소요시간_분', '협업비율(%)', '_tf', '_te', '_ca', '_ac'])
            ...
            with pa2:
                ...
                coop_top['_ca'] = coop_top['조치자'].apply(lambda w: _ca_worker(w, worker_df))
                coop_top['_ac'] = coop_top['조치자'].apply(lambda w: _ac_worker(w, worker_df))
                fig_coop = px.bar(
                    coop_top, x='조치자', y='협업비율(%)',
                    color='협업비율(%)', color_continuous_scale='Oranges',
                    custom_data=['출동건수', '단독콜', '협업콜', '_tf', '_te', '_ca', '_ac'])
            ...
            with st.expander('📋 Top3 상세 — 협업비율 상위 전체 항목', expanded=True):
                def _s(t): return ' | '.join(p.strip() for p in t.split('<br>') if p.strip()) if t and t != '-' else '-'
                _disp6 = pd.DataFrame({
                    '담당자': coop_top['조치자'].values,
                    '주요 고장유형 Top3': coop_top['_tf'].apply(_s).values,
                    '원인 Top3':           coop_top['_ca'].apply(_s).values,
                    '조치내역 Top3':       coop_top['_ac'].apply(_s).values,
                    '담당 설비 Top3':      coop_top['_te'].apply(_s).values,
                })
                _disp6.index += 1
                st.dataframe(_disp6, use_container_width=True, height=min(420, (len(_disp6)+1)*35+3))
                if st.button('📥 Excel 다운로드', key='xl_coop'):
                    _xl6 = _disp6.copy()
                    for _c in _xl6.columns[1:]: _xl6[_c] = _xl6[_c].str.replace(' | ', '\n', regex=False)
                    _xout6 = to_excel({'협업비율Top3': _xl6})
                    st.download_button('⬇️ 다운로드', data=_xout6,
                                       file_name=f"협업비율Top3_{datetime.now().strftime('%Y%m%d')}.xlsx",
                                       mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                                       key='dldl_coop')
            # ── 협업 필요 설비 Top N ───────────────────
...
if _page == "🏆 설비 위험도":
    ...
    if df is None:
        ...
        risk_top['_ca'] = risk_top.apply(lambda r: _ca_risk(r, rdf), axis=1)
        risk_top['_ac'] = risk_top.apply(lambda r: _ac_risk(r, rdf), axis=1)
        ...
        fig_risk = px.bar(risk_top.sort_values('위험도점수'),x='위험도점수',y='설비KEY',orientation='h',
                          ...
                          custom_data=['건수','총정지시간','평균MTTR','설비유형','_tf','_te','_ca','_ac'])
        fig_risk.update_traces(
            hovertemplate=('<b>%{y}</b><br>위험도: %{x:.1f}점<br>설비유형: %{customdata[3]}<br>'
                           '건수: %{customdata[0]}건<br>총정지: %{customdata[1]:,.0f}분<br>'
            '<b>▶ 반복 고장 Top 3 (부위/현상)</b><br>%{customdata[4]}<br>'
            '<b>▶ 주요 원인 Top 3</b><br>%{customdata[6]}<br>'
            '<b>▶ 주요 조치 Top 3</b><br>%{customdata[7]}<br>'
        ...
        with st.expander('📋 Top3 상세 — 위험도 전체 항목', expanded=True):
            def _s(t): return ' | '.join(p.strip() for p in t.split('<br>') if p.strip()) if t and t != '-' else '-'
            _disp7 = pd.DataFrame({
                '설비': risk_top['설비KEY'].values,
                '부위/현상 Top3': risk_top['_tf'].apply(_s).values,
                '원인 Top3':       risk_top['_ca'].apply(_s).values,
                '조치내역 Top3':   risk_top['_ac'].apply(_s).values,
                '조치자 Top3':     risk_top['_te'].apply(_s).values,
            })
            _disp7.index += 1
            st.dataframe(_disp7, use_container_width=True, height=min(420, (len(_disp7)+1)*35+3))
            if st.button('📥 Excel 다운로드', key='xl_risk'):
                _xl7 = _disp7.copy()
                for _c in _xl7.columns[1:]: _xl7[_c] = _xl7[_c].str.replace(' | ', '\n', regex=False)
                _xout7 = to_excel({'위험도Top3': _xl7})
                st.download_button('⬇️ 다운로드', data=_xout7,
                                   file_name=f"위험도Top3_{datetime.now().strftime('%Y%m%d')}.xlsx",
                                   mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                                   key='dldl_risk')
...
if _page == "🔁 재발 고장 전용":
    ...
    if df is None:
        ...
        top_recur['_ca'] = top_recur['설비_KEY'].apply(lambda k: _ca_rr(k, rdf))
        top_recur['_ac'] = top_recur['설비_KEY'].apply(lambda k: _ac_rr(k, rdf))
        fig_rr = px.bar(top_recur.sort_values('재발률(%)'),
                        x='재발률(%)',y='설비_KEY',orientation='h',color='등급',
                        color_discrete_map=color_map_r,
                        custom_data=['전체건수','재발건수','설비유형','_tf','_te','_ca','_ac'])
        ...
        with st.expander('📋 Top3 상세 — 재발률 전체 항목', expanded=True):
            def _s(t): return ' | '.join(p.strip() for p in t.split('<br>') if p.strip()) if t and t != '-' else '-'
            _disp8 = pd.DataFrame({
                '설비': top_recur['설비_KEY'].values,
                '부위/현상 Top3': top_recur['_tf'].apply(_s).values,
                '원인 Top3':       top_recur['_ca'].apply(_s).values,
                '조치내역 Top3':   top_recur['_ac'].apply(_s).values,
                '조치자 Top3':     top_recur['_te'].apply(_s).values,
            })
            _disp8.index += 1
            st.dataframe(_disp8, use_container_width=True, height=min(420, (len(_disp8)+1)*35+3))
            if st.button('📥 Excel 다운로드', key='xl_rr'):
                _xl8 = _disp8.copy()
                for _c in _xl8.columns[1:]: _xl8[_c] = _xl8[_c].str.replace(' | ', '\n', regex=False)
                _xout8 = to_excel({'재발률Top3': _xl8})
                st.download_button('⬇️ 다운로드', data=_xout8,
                                   file_name=f"재발률Top3_{datetime.now().strftime('%Y%m%d')}.xlsx",
                                   mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                                   key='dldl_rr')

        # ── 재발 간격 상세 분석 ──
        st.markdown("##### 동일 설비·동일 부위 재발 간격 분석")
        gap_df = _gap_analysis(rdf)
        if not gap_df.empty:
...
if _page == "🏷️ 표준코드 분석":
    ...
    if df is None:
        ...
        if sdf.empty:
            ...
            if '조치코드' in sdf.columns:
                parts_result = _parts_cycle(sdf)
                if not parts_result.empty:
...
if _page == "📖 사용설명서":
    _print_tab('사용설명서')
    st.markdown('<div class="main-title">📖 사용설명서</div>', unsafe_allow_html=True)
    st.markdown('<div class="sub-title">보전팀 통합 분석 시스템 사용 안내</div>', unsafe_allow_html=True)

    _m1, _m2, _m3, _m4, _m5, _m6 = st.tabs([
        '1장. 프로그램 개요',
        '2장. 장점 및 단점',
        '3장. 파일 업로드',
        '4장. 탭별 사용 방법',
...
보전팀 통합 분석 시스템은 제조 현장의 설비 고장 이력 데이터를 업로드하여
고장 현황, MTTR/MTBF, 인원 부하, KPI 목표 대비 실적, 재발 고장, 월보·주보 등을
한 화면에서 분석하고 출력할 수 있는 Streamlit 기반 웹 분석 도구입니다.

---
### 1.2 지원 데이터 형식 (3가지 자동 감지)

| 구분 | 시트명 | 컬럼 수 | 식별 기준 컬럼 |
|------|--------|---------|--------------|
| 프레스 데이터 | 설비보전현황_통합 | 17개 | 년, 월, 라인, 설비유형 |
| 로봇/지그 데이터 | Sheet1 | 21개 | 라인_KEY, 차종, 고장부위_STD |
| 광주공장 원본 | Sheet1 | 14개 | 구분, 라인명, 소요시각 |

---
### 1.3 프레스 데이터 컬럼 구조 (17개)
...
|------|------|
| Python 버전 | 3.9 이상 |
| 필수 패키지 | streamlit, pandas, numpy, plotly, openpyxl, requests |
| 실행 명령 | streamlit run app.py |
| 권장 브라우저 | Chrome 또는 Edge 최신 버전 |
| 인터넷 연결 | Google Drive 연동 사용 시 필요 |
""")
...
| 번호 | 기능 | 설명 |
|------|------|------|
| 1 | 엑셀 포맷 자동 감지 | 업로드 시 컬럼 구조를 자동 인식, 별도 변환 없이 바로 분석 가능 (광주 원본 포함) |
| 2 | 15개 탭 원스톱 분석 | 고장현황·MTTR/MTBF·인원부하·KPI·재발고장·월보/주보를 한 화면에서 통합 관리 |
| 3 | 자동 표준코드 분류 | 현상/원인/조치 텍스트 기반으로 고장계통코드·원인코드·조치코드 자동 생성 |
| 4 | 작업자명 오타 자동 수정 | 유사도 0.65 기준으로 오입력 이름을 기준 이름으로 자동 정규화 (35명 등록) |
| 5 | Google Drive 연동 | 공유 링크 붙여넣기만으로 구글드라이브 파일 자동 다운로드 |
| 6 | 월보·주보 자동 생성 | 전기 대비 변동률, Top5 설비, KPI 비교표 한 클릭 생성 |
| 7 | 다양한 출력 형식 | Excel 멀티시트 / CSV / HTML 보고서 / 탭별 PDF 인쇄 지원 |
| 8 | 재발 고장 전문 분석 | 30·60·90·180일 윈도우 선택, 재발간격 등급 카드 자동 표시 |
| 9 | KPI 목표 관리 | 고장건수·MTTR·총정지시간·MTBF 목표값 설정 및 달성률 시각화 |
| 10 | 탭별 인쇄 최적화 | 현재 탭만 A4/A3 가로 인쇄 가능 (비활성 탭 자동 숨김) |

---
### 2.2 단점 및 주의사항
""")
        st.warning('아래 사항을 반드시 확인하세요.')
        st.markdown("""
| 번호 | 주의 항목 | 내용 |
|------|-----------|------|
| 1 | 컬럼 순서 변경 불가 | 각 포맷별 컬럼 위치가 고정 — 엑셀에서 컬럼 추가·이동 시 오류 발생 |
| 2 | 세션 초기화 시 데이터 소실 | 브라우저 새로고침 또는 재접속 시 업로드 데이터·KPI 설정값 초기화 |
| 3 | 작업자 목록 코드 수정 필요 | 신규 인원 추가·퇴사자 제거 시 app.py의 VALID_WORKERS 리스트 직접 편집 필요 |
| 4 | Google Drive 공유 설정 필요 | 파일을 링크 있는 모든 사용자 공유로 설정, 타임아웃 30초 |
| 5 | 소요시간 24시간 초과 자동 제외 | 1440분 초과 값은 무효 처리 — 장기 수리 이력 누락 가능 |
| 6 | 광주원본 차종 필터 미작동 | 라인명에서 차종을 분리하지 않아 차종 필터가 빈값으로 표시됨 |
| 7 | 대용량 데이터 처리 지연 | 수만 건 이상 시 재발 판정·클러스터링 계산 시간 증가 |
""")
...
| 단계 | 방법 |
|------|------|
| 1단계 | 프로그램 상단 **데이터 불러오기** 탭 클릭 |
| 2단계 | 파일 업로드 영역에서 **Browse files** 클릭 또는 파일 드래그 앤 드롭 |
| 3단계 | 프레스 파일과 로봇/지그 파일 동시 선택 가능 (여러 파일 동시 업로드) |
| 4단계 | 자동 감지 완료 후 **데이터 통합 실행** 버튼 클릭 |
| 5단계 | 통합 완료 메시지 및 미리보기 데이터 확인 |

---
### 3.3 Google Drive 연동 방법

| 단계 | 방법 |
|------|------|
| 1단계 | Google Drive에서 파일 우클릭 → 공유 → **링크 있는 모든 사용자** 설정 |
| 2단계 | 링크 복사 클릭 |
| 3단계 | 프로그램 **Google Drive URL** 입력창에 붙여넣기 |
| 4단계 | **드라이브에서 불러오기** 버튼 클릭 |
| 5단계 | 다운로드 완료 후 **데이터 통합 실행** 클릭 |
...
| 항목 | 의미 |
|------|------|
| 발생일시 누락 | 날짜/시각 정보가 없는 행 수 |
| 소요시간 누락 | 정지시간 정보가 없는 행 수 |
| 설비유형 누락 | 설비 분류가 빈값인 행 수 |
| 중복 건수 | 동일 일시·설비·소요시간 중복 행 수 |
""")
...
| 탭 | 이름 | 핵심 기능 |
|----|------|---------|
| 1 | 데이터 불러오기 | 파일 업로드 / 드라이브 연동 / 데이터 통합 |
| 2 | 고장현황 Pareto | 설비·라인별 파레토, KPI 4종, 호버 상세 |
| 3 | 설비분석 MTTR/MTBF | 복구시간·고장간격 산점도, 데이터품질 다운로드 |
| 4 | 인원분석 | 단독/협업 비율, 인원별 부하 순위 |
| 5 | 설비 위험도 | 가중치 조정 위험도 점수, 등급별 색상 카드 |
| 6 | 유실시간 분석 | 월별 유실시간 이중축 차트 |
| 7 | 예방정비 추천 | 우선순위 3등급, Excel 다운로드 |
| 8 | 월별 트렌드 | 이동평균, 히트맵, 연도별 비교 |
| 9 | KPI 목표관리 | 목표값 설정, 달성률 게이지 |
| 10 | BM/PM 분석 | 돌발·계획 비율, 야간 고장률 |
| 11 | 재발 고장 전용 | 윈도우 선택, 재발간격 등급 카드 |
| 12 | 표준코드 분석 | 고장계통·원인·조치코드 파레토 |
| 13 | 월보·주보 자동작성 | 전기 대비 자동 비교 보고서 |
| 14 | 출력 | Excel·CSV·HTML·PDF 다운로드 |
""")
        st.divider()
        _t_items = [
            ('TAB 1 — 데이터 불러오기', [
                ('목적', '엑셀 파일 업로드 및 데이터 통합'),
                ('주요 기능', '로컬 파일 업로드(xlsx 복수선택) / Google Drive 링크 다운로드 / 자동 감지 및 통합 / 미리보기(상위 200건) / 품질 진단'),
                ('주의', '반드시 데이터 통합 실행 버튼을 눌러야 다른 탭에서 데이터 사용 가능'),
            ]),
            ('TAB 2 — 고장현황 (Pareto)', [
                ('목적', '설비·라인별 고장 건수와 정지시간 파레토 분석'),
                ('주요 필터', '설비유형, 라인, 차종, Top N (5~30)'),
                ('KPI 4종', '총 고장건수 / 총 정지시간(분) / 평균 MTTR(분) / 평균 응답시간(분)'),
                ('차트', '이중축 파레토(건수 막대+누적% 선), 라인별 수평 막대'),
                ('호버 정보', '반복 고장 Top3, 고장 다발 설비 Top3'),
            ]),
            ('TAB 3 — 설비분석 (MTTR/MTBF)', [
                ('목적', '설비별 평균복구시간(MTTR)과 평균고장간격(MTBF) 분석'),
                ('주요 필터', '설비유형, Top N(10~50), 클러스터링 임계값(30~240분, 기본 60분)'),
                ('클러스터링', '완료 후 60분 이내 동일 설비 재출동은 1건으로 통합'),
                ('차트', 'MTTR 수평 막대 / MTTR×MTBF 산점도 / 응답시간 히스토그램'),
                ('다운로드', '데이터 품질 이슈 Excel 파일'),
            ]),
            ('TAB 4 — 인원분석', [
                ('목적', '조치자별 출동 건수·소요시간·단독/협업 비율 분석'),
                ('차트', '단독/협업 스택 바, 총소요시간 순위, 협업 비율 순위'),
                ('경고', '협업 비율 70% 초과 인원 강조 표시'),
                ('호버 정보', '주요 고장유형 Top3, 담당 설비 Top3'),
            ]),
            ('TAB 5 — 설비 위험도', [
                ('목적', '빈도·정지시간·MTTR 가중치 기반 설비 위험도 점수 산출'),
                ('가중치', '빈도 40% / 정지시간 40% / MTTR 20% (슬라이더 조정 가능)'),
                ('위험 등급', '빨강: 즉시조치 / 주황: 주의 / 초록: 관찰'),
                ('활용', '가중치를 조정해 현장 상황에 맞는 우선순위 도출'),
            ]),
            ('TAB 6 — 유실시간 분석', [
                ('목적', '월별 설비 가동 손실 시간 추이 분석'),
                ('차트', '월별 유실시간 이중축(막대+건수 선그래프)'),
                ('필터', '설비유형, 차종·라인'),
            ]),
            ('TAB 7 — 예방정비 추천', [
                ('목적', '고장 이력 기반 예방정비 우선순위 자동 산출'),
                ('우선순위 등급', '빨강: 즉시조치(상위 20%) / 주황: 조기예방(20~60%) / 초록: 정기점검(하위 40%)'),
                ('자동 추천', '점검주기 단축(30일 미만) / 월간 집중점검(30~90일) / 예비부품 확보(MTTR 기준)'),
                ('다운로드', '예방정비 추천 Excel 버튼으로 결과 파일 다운로드'),
            ]),
            ('TAB 8 — 월별 트렌드', [
                ('목적', '월별 고장 건수 추이 및 연도 비교'),
                ('차트', '월별 건수 막대+3개월 이동평균 / 설비유형별 스택 바 / 라인×월 히트맵 / 연도별 비교 라인'),
                ('활용', '계절성 고장 패턴 및 전년도 동기 대비 증감 확인'),
            ]),
            ('TAB 9 — KPI 목표관리', [
                ('목적', 'KPI 목표값 설정 및 실적 달성 현황 확인'),
                ('KPI 항목', '고장건수(기본 100건) / 평균 MTTR(기본 30분) / 총정지시간(기본 3,000분) / 평균 MTBF(기본 200시간)'),
                ('주의', 'KPI 목표값은 세션에만 저장 — 브라우저 새로고침 시 기본값으로 초기화'),
            ]),
            ('TAB 10 — BM/PM 분석', [
                ('목적', '돌발(BM) vs 계획(PM) 정비 비율 및 야간 고장률 분석'),
                ('PM 목표', '30% 이상 시 달성 표시'),
                ('차트', 'BM/PM 도넛 차트 / 설비유형별 스택 바 / 월별 BM/PM 비율 추이 / 야간(22시~6시) vs 주간 비교'),
            ]),
            ('TAB 11 — 재발 고장 전용', [
                ('목적', '동일 설비·부위에서 반복 발생하는 재발 고장 전문 분석'),
                ('재발 판정 윈도우', '30일 / 60일 / 90일 / 180일 선택 가능'),
                ('재발률 등급', '매우위험: 50% 이상 / 위험: 30~50% / 주의: 15~30% / 양호: 15% 미만'),
                ('위험 카드', '평균 재발간격 2주 이내 설비 빨간 카드 강조, 근본대책 수립 안내'),
            ]),
            ('TAB 12 — 표준코드 분석', [
                ('목적', '고장계통코드·원인코드·조치코드 기반 파레토 분석'),
                ('표시', '자동 분류된 표준코드 3열 파레토 차트 동시 표시 (80% 라인 포함)'),
                ('활용', '반복 발생 고장 계통 식별 및 중점 개선 방향 설정'),
            ]),
            ('TAB 13 — 월보·주보 자동작성', [
                ('목적', '전기 대비 주요 지표 비교 보고서 자동 생성'),
                ('선택', '월보 또는 주보 라디오 버튼 선택'),
                ('자동 생성 항목', '총괄(고장건수·정지시간·MTTR 변동) / BM/PM 현황 / 설비유형별 Top5 / 라인별 Top5 / 최장정지 Top5 / 차기 조치사항'),
            ]),
            ('TAB 14 — 출력', [
                ('목적', '분석 결과를 다양한 형식으로 다운로드'),
                ('Excel', '보전팀_분석_날짜시간.xlsx — 멀티시트(통합데이터·Pareto·MTTR·인원·코드·재발·품질)'),
                ('CSV', '보전팀_통합_날짜시간.csv — 통합 데이터 전체(UTF-8)'),
                ('HTML', '보전팀_분석보고서_날짜시간.html — 설비유형·라인별 테이블 포함'),
                ('PDF', '각 탭 상단 인쇄 버튼 클릭 — 현재 탭 내용만 A3 가로 인쇄 권장'),
            ]),
        ]
        for _title, _rows in _t_items:
...
| 코드 | 분류 기준 키워드 예시 |
|------|-------------------|
| 전기 | 케이블, 단선, 전원, 퓨즈, EOCR, 인버터, 누전, 차단기, 배선 |
| 제어 | PLC, 프로그램, 통신, 로직, 파라미터, 티칭, 신호 |
| 유압 | 오일, 유압, 누유, 펌프, 밸브, 압력, 실린더, 호스 |
| 공압 | AIR, 에어, 공압, 컴프레셔, 솔레노이드, 튜브 |
| 기계 | 베어링, 볼트, 마모, 균열, 파손, 벨트, 체인, 기어, 용접 |
| 센서 | 센서, L/S, 리미트, 근접, 광전, 포토, 엔코더 |
| 안전 | 안전, 비상, Light Curtain, 도어, 인터락, Guard |
| 윤활 | 그리스, 윤활, 오일팬, 급유 |
...
| 코드 | 분류 기준 키워드 예시 |
|------|-------------------|
| 마모 | 마모, 닳음, 파손, 변형, 균열, 크랙 |
| 단선 | 단선, 단락, 케이블 끊김, 접촉불량 |
| 오염 | 오염, 먼지, 이물, 슬러지, 막힘 |
| 간섭 | 간섭, 충돌, 걸림, Jamming |
| 설정값이탈 | 설정, 파라미터, 티칭, 원점, 캘리브레이션 |
| 품질기인 | 품질, 이종, 버(Burr), 가공 불량 |
| 작업자기인 | 작업자 조작, 인위적 원인, 실수 |
| 자연열화 | 열화, 노후, 수명, 교체주기 |
...
| 코드 | 분류 기준 키워드 예시 |
|------|-------------------|
| 교체 | 교체, 교환, 신품, Spare, 부품 교환 |
| 수리 | 수리, 용접, 패치, 보수 |
| 조정 | 조정, 세팅, 설정, 재조임 |
| 티칭 | 티칭, Teaching, 원점, 원위치 |
| 청소 | 청소, 세척, 이물 제거, 에어블로 |
| 리셋 | 리셋, Reset, 재기동, 재가동, 원복 |
| 개선 | 개선, 방지대책, 영구 대책 |
| 예방 | 예방, PM, 계획정비, 정기점검 |
...
| 구분 | 판정 조건 |
|------|---------|
| PM(계획) | 조치유형이 예방보전이거나, 현상·원인·조치내역에 예방·PM·계획정비·정기점검 포함 |
| BM(돌발) | 위 조건에 해당하지 않는 모든 건 |
""")
    ...
    with _m6:
        ...
        _faqs = [
            ('Q1.  파일 업로드 후 인식불가 오류가 나옵니다.',
             '- 파일 형식이 **.xlsx** 인지 확인 (xls, csv는 변환 필요)\n'
             '- **프레스**: 시트명 `설비보전현황_통합` 또는 컬럼에 년·월·라인·설비유형 포함\n'
             '- **로봇/지그**: 시트명 `Sheet1`에 라인_KEY, 차종, 고장부위_STD 중 하나 이상\n'
             '- **광주원본**: 시트명 `Sheet1`에 구분, 라인명, 소요시각 중 하나 이상'),
            ('Q2.  특정 작업자 이름이 인원분석에 표시되지 않습니다.',
             'VALID_WORKERS 리스트에 등록된 **35명**만 분석 대상입니다.\n'
             '신규 인원은 app.py의 VALID_WORKERS 리스트에 이름을 직접 추가하세요.\n'
             '유사도 0.65 미만의 오타는 인식 불가 처리됩니다.'),
            ('Q3.  Google Drive 파일을 불러올 수 없습니다.',
             '- 공유 설정을 **링크 있는 모든 사용자 — 뷰어**로 변경하세요.\n'
             '- 인터넷 연결 상태를 확인하세요.\n'
             '- 타임아웃(30초) 초과 시 재시도하세요.\n'
             '- 회사 방화벽 차단 시 로컬 업로드 방식을 사용하세요.'),
            ('Q4.  소요시간이 0 또는 비어 있게 나옵니다.',
             '소요시간 미입력 시 출동시각~완료시각 차이로 자동 계산합니다.\n'
             '두 시각 모두 빈값이면 결측값으로 처리됩니다.\n'
             '**1440분(24시간) 초과 값은 자동으로 무효 처리**됩니다.\n'
             '원본 데이터에서 시각 정보가 정확히 입력되어 있는지 확인하세요.'),
            ('Q5.  브라우저 새로고침 후 데이터가 사라졌습니다.',
             'Streamlit 세션 기반으로 동작하여 새로고침 시 데이터가 초기화됩니다.\n'
             '분석 완료 후 **출력 탭**에서 Excel 또는 CSV로 저장해두세요.'),
            ('Q6.  차종 필터가 빈값만 표시됩니다. (광주 원본 사용 시)',
             '광주 원본 14컬럼은 라인명에서 차종을 자동 분리하지 않습니다.\n'
             '라인 기준으로 분석은 정상 동작합니다.'),
            ('Q7.  같은 데이터가 두 번 집계되는 것 같습니다.',
             '같은 파일을 두 번 업로드하지 않도록 주의하세요.\n'
             '데이터 품질 진단 화면의 **중복 건수**를 확인하면 됩니다.'),
        ]
        for _q, _a in _faqs:
            with st.expander(_q):
                st.markdown(_a)
...
# ══════════════════════════════════════════════════════
# TAB 17 — 💡 인사이트 대시보드 (Equipment Maintenance 차용 9종)
# ══════════════════════════════════════════════════════
if _page == "💡 인사이트":
    df = st.session_state.merged_df
    if df is None:
        ...
        st.caption("선택된 전역 기간 필터 기준. 급증·비교는 데이터 내 최신일 기준으로 계산됩니다.")

        idf = apply_global_filter(df)
        idf = idf[idf['발생일시'].notna()].copy()
        ...
        if idf.empty:
            ...
            now = idf['발생일시'].max()
            total_i = len(idf)
            idf_v = idf[idf['소요시간'].notna()]

            # ───────────────────────────────────────────
            # 공통 helper
            # ───────────────────────────────────────────
            def _i_unresolved(d):
                if '조치내역' not in d.columns: return 0
                c = d['조치내역'].astype(str).str.strip()
                return int(((c == '') | (c == 'nan') | (c == 'None')).sum())

            # ① 급증 감지 (캐시 함수) ─────────────────────
            surge = _detect_surge_cached(idf)

            # ② 자동 인사이트 문장 ──────────────────────
            st.markdown("### 📝 자동 인사이트")
            _ins = [auto_summary(idf, '선택 기간')]
            if not surge.empty:
                _top_s = surge.iloc[0]
                _ins.append(f"🚨 **급증 설비 {len(surge)}개** 감지 — 최다 **{_top_s['설비']}** "
                            f"(최근 7일 {int(_top_s['최근7일'])}건).")
            # 집중 시간대
            _hr = idf['발생일시'].dt.hour
            if not _hr.empty:
                _peak_h = int(_hr.value_counts().idxmax())
                _peak_c = int(_hr.value_counts().max())
                _ins.append(f"⏰ 고장 집중 시간대 **{_peak_h}시** ({_peak_c}건).")
            _unres = _i_unresolved(idf)
            if total_i:
                _ins.append(f"🔧 미해결(조치 미입력) **{_unres}건** ({_unres/total_i*100:.0f}%).")
            st.info("\n\n".join(_ins))
            st.divider()

            # ③ 전월 대비 KPI ───────────────────────────
            st.markdown("### 📊 전월 대비 KPI")
            _ly = int(idf['발생일시'].dt.year.max())
            _lm = int(idf[idf['발생일시'].dt.year == _ly]['발생일시'].dt.month.max())
            cur_cnt, prev_cnt, cur_stop, prev_stop = calc_mom_delta(df, _ly, _lm)
            # 당월/전월 MTTR·재발률
            _dd = df[df['발생일시'].notna()].copy()
            _dd['년'] = _dd['발생일시'].dt.year; _dd['월'] = _dd['발생일시'].dt.month
            _cur = _dd[(_dd['년'] == _ly) & (_dd['월'] == _lm)]
            _pyr = _ly if _lm > 1 else _ly - 1
            _pmo = _lm - 1 if _lm > 1 else 12
            _prev = _dd[(_dd['년'] == _pyr) & (_dd['월'] == _pmo)]
            cur_mttr = _cur['소요시간'].mean() if not _cur.empty else 0
            prev_mttr = _prev['소요시간'].mean() if not _prev.empty else 0
            cur_re = _cur['재발여부'].mean() * 100 if ('재발여부' in _cur.columns and not _cur.empty) else 0
            prev_re = _prev['재발여부'].mean() * 100 if ('재발여부' in _prev.columns and not _prev.empty) else 0

            def _delta_txt(cur, prev, unit='', invert=True):
                if prev == 0:
                    return "신규"
                d = (cur - prev) / prev * 100
            ...
            st.caption(f"기준월: {_ly}년 {_lm}월 (vs {_pyr}년 {_pmo}월)")
            mk1, mk2, mk3 = st.columns(3)
            mk1.metric(f"{_lm}월 고장건수", f"{cur_cnt:,}건",
                       delta=f"{cur_cnt-prev_cnt:+,}건 (전월 {prev_cnt:,})", delta_color='inverse')
            mk2.metric(f"{_lm}월 평균 MTTR", f"{cur_mttr:.0f}분",
                       delta=f"{cur_mttr-prev_mttr:+.0f}분 (전월 {prev_mttr:.0f})", delta_color='inverse')
            mk3.metric(f"{_lm}월 재발률", f"{cur_re:.1f}%",
                       delta=f"{cur_re-prev_re:+.1f}%p (전월 {prev_re:.1f})", delta_color='inverse')
            st.divider()

            # ① 급증 감지 표시 ──────────────────────────
            st.markdown("### 🚨 급증 감지 (최근 7일 ≥3건 & 평소 2배↑)")
            if surge.empty:
                st.success("급증 설비가 없습니다. 안정적입니다.")
            else:
                _sshow = surge.copy()
                _sshow['배수'] = _sshow['배수'].apply(lambda v: '신규/급증' if v == float('inf') else f"{v}배")
                st.dataframe(_sshow, use_container_width=True,
                             height=min(360, (len(_sshow)+1)*35+3))
            st.divider()

            # ④ 유실시간 파레토 ─────────────────────────
            st.markdown("### ⏱ 유실시간 파레토 (정지시간 누적 80% 핵심설비)")
            _loss = (idf_v.groupby('설비_KEY')['소요시간'].sum()
                     .sort_values(ascending=False).reset_index())
            _loss.columns = ['설비', '총정지시간(분)']
            if not _loss.empty and _loss['총정지시간(분)'].sum() > 0:
                _loss['누적%'] = (_loss['총정지시간(분)'].cumsum() / _loss['총정지시간(분)'].sum() * 100).round(1)
                _loss['핵심'] = _loss['누적%'].apply(lambda p: '🔴 핵심(80%내)' if p <= 80 else '⚪')
                _lt = _loss.head(15)
                fig_loss = make_subplots(specs=[[{"secondary_y": True}]])
                fig_loss.add_trace(go.Bar(x=_lt['설비'], y=_lt['총정지시간(분)'], name='정지시간',
                                          marker_color='#c0392b'), secondary_y=False)
                fig_loss.add_trace(go.Scatter(x=_lt['설비'], y=_lt['누적%'], name='누적%',
                                              line=dict(color='#e67e22', width=2), mode='lines+markers'),
                                   secondary_y=True)
                fig_loss.add_hline(y=80, line_dash='dash', line_color='orange', secondary_y=True)
                fig_loss.update_yaxes(title_text='정지시간(분)', secondary_y=False)
                fig_loss.update_yaxes(title_text='누적 %', range=[0, 105], secondary_y=True)
                fig_loss.update_layout(height=380, margin=dict(t=20, b=120, l=10, r=60),
                                       xaxis_tickangle=-40,
                                       legend=dict(orientation='h', y=1.05))
                st.plotly_chart(fig_loss, use_container_width=True)
                _core_n = int((_loss['누적%'] <= 80).sum())
                st.caption(f"전체 {len(_loss)}개 설비 중 상위 **{_core_n}개**가 총 정지시간의 80%를 차지합니다.")
                with st.expander("📋 유실시간 전체 + Excel"):
                    st.dataframe(_loss, use_container_width=True, height=320)
                    if st.button("📥 Excel 다운로드", key='t17_loss_xl'):
                        _o = to_excel({'유실파레토': _loss})
                        st.download_button("⬇️ 다운로드", data=_o,
                                           file_name=f"유실파레토_{datetime.now().strftime('%Y%m%d')}.xlsx",
                                           mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                                           key='t17_loss_dl')
            else:
                st.info("정지시간 데이터가 없습니다.")
            st.divider()

            # ⑤ 주요 교체부품 빈도 ──────────────────────
            st.markdown("### 🧩 주요 교체부품 빈도 (원인·조치 텍스트 추출)")
            PART_WORDS = ['센서', '핀', '팁', '케이블', '밸브', '솔밸브', '실린더', '호스', '모터',
                          '베어링', '벨트', '체인', '기어', '펌프', '실링', '패킹', '오링', '커플링',
                          '노즐', '퓨즈', '릴레이', '스위치', '커넥터', '볼트', '너트', '그리퍼',
                          '척', '클램프', '스프링', '가이드', '롤러', '기판', '컨트롤러', '배선',
                          '하네스', '실리콘', '게이지', '레귤레이터', '필터', '오일', '그리스']
            _txt = (idf.get('원인', pd.Series(dtype=str)).astype(str).fillna('') + ' '
                    + idf.get('조치내역', pd.Series(dtype=str)).astype(str).fillna('')).str.lower()
            _pc = [(w, int(_txt.str.contains(w.lower(), regex=False).sum())) for w in PART_WORDS]
            _pc = [(w, c) for w, c in _pc if c > 0]
            _pc.sort(key=lambda x: x[1], reverse=True)
            if _pc:
                _pdf = pd.DataFrame(_pc, columns=['부품', '언급건수']).head(20)
                fig_part = px.bar(_pdf.sort_values('언급건수'), x='언급건수', y='부품',
                                  orientation='h', color='언급건수', color_continuous_scale='Tealgrn')
                fig_part.update_layout(height=max(300, len(_pdf)*24), margin=dict(t=20, b=20, l=10, r=40),
                                       coloraxis_showscale=False, yaxis_title='')
                st.plotly_chart(fig_part, use_container_width=True)
                st.caption("예비품 재고 우선순위·발주 참고. 텍스트 부분일치 기준이라 중복 카운트 가능합니다.")
            else:
                st.info("부품 키워드가 감지되지 않았습니다.")
            st.divider()

            # ⑦ 시간대·요일 발생패턴 ────────────────────
            st.markdown("### 🕐 시간대 · 요일 발생패턴")
            pc1, pc2 = st.columns(2)
            with pc1:
                _hh = idf['발생일시'].dt.hour.value_counts().reindex(range(24), fill_value=0)
                fig_hh = px.bar(x=list(range(24)), y=_hh.values,
                                labels={'x': '시', 'y': '건수'}, color=_hh.values,
                                color_continuous_scale='Reds')
                fig_hh.update_layout(height=300, margin=dict(t=20, b=20), coloraxis_showscale=False,
                                     title='시간대별 (0~23시)')
                st.plotly_chart(fig_hh, use_container_width=True)
            with pc2:
                _wd_map = ['월', '화', '수', '목', '금', '토', '일']
                _wd = idf['발생일시'].dt.weekday.value_counts().reindex(range(7), fill_value=0)
                fig_wd = px.bar(x=_wd_map, y=_wd.values,
                                labels={'x': '요일', 'y': '건수'}, color=_wd.values,
                                color_continuous_scale='Blues')
                fig_wd.update_layout(height=300, margin=dict(t=20, b=20), coloraxis_showscale=False,
                                     title='요일별')
                st.plotly_chart(fig_wd, use_container_width=True)
            st.divider()

            # ⑧ 라인×부위 히트맵 ────────────────────────
            st.markdown("### 🔥 라인 × 부위 히트맵 (상위 교차)")
            _bcol = '부위그룹' if '부위그룹' in idf.columns else '고장부위'
            if _bcol in idf.columns and '라인_차종' in idf.columns:
                _top_ln = idf['라인_차종'].value_counts().head(8).index.tolist()
                _top_bu = idf[_bcol].value_counts().head(8).index.tolist()
                _hm = idf[idf['라인_차종'].isin(_top_ln) & idf[_bcol].isin(_top_bu)]
                if not _hm.empty:
                    _pivot = (_hm.groupby(['라인_차종', _bcol]).size()
                              .reset_index(name='건수')
                              .pivot(index='라인_차종', columns=_bcol, values='건수').fillna(0).astype(int))
                    fig_hm = px.imshow(_pivot, text_auto=True, aspect='auto',
                                       color_continuous_scale='OrRd',
                                       labels=dict(x='부위', y='라인', color='건수'))
                    fig_hm.update_layout(height=max(320, len(_pivot)*40), margin=dict(t=20, b=20))
                    st.plotly_chart(fig_hm, use_container_width=True)
                    st.caption("색이 진할수록 해당 라인·부위 고장이 잦습니다. 클릭 대상 우선순위 참고.")
                else:
                    st.info("교차 데이터가 부족합니다.")
            st.divider()

            # ⑥ 기간 비교보고서 ─────────────────────────
            st.markdown("### 🔁 기간 비교보고서")
            _preset = st.selectbox("비교 프리셋", ['이번달 vs 저번달', '최근 7일 vs 이전 7일',
                                                   '최근 30일 vs 이전 30일', '최근 90일 vs 이전 90일'],
                                   key='t17_cmp')
            _full = df[df['발생일시'].notna()].copy()
            if _preset == '이번달 vs 저번달':
                _fa = _full[(_full['발생일시'].dt.year == _ly) & (_full['발생일시'].dt.month == _lm)]
                _fb = _full[(_full['발생일시'].dt.year == _pyr) & (_full['발생일시'].dt.month == _pmo)]
                _la, _lb = f"{_ly}-{_lm:02d}", f"{_pyr}-{_pmo:02d}"
            else:
                _days = {'최근 7일 vs 이전 7일': 7, '최근 30일 vs 이전 30일': 30,
                         '최근 90일 vs 이전 90일': 90}[_preset]
                _a0 = now - timedelta(days=_days)
                _b0 = now - timedelta(days=_days * 2)
                _fa = _full[_full['발생일시'] > _a0]
                _fb = _full[(_full['발생일시'] > _b0) & (_full['발생일시'] <= _a0)]
                _la, _lb = f"최근{_days}일", f"이전{_days}일"

            def _kpi_row(d):
                v = d[d['소요시간'].notna()]
                return {
                    '고장건수': len(d),
                    '총정지시간(분)': int(v['소요시간'].sum()) if not v.empty else 0,
                    '평균MTTR(분)': round(v['소요시간'].mean(), 1) if not v.empty else 0,
                    '재발률(%)': round(d['재발여부'].mean()*100, 1) if ('재발여부' in d.columns and len(d)) else 0,
                    '미해결(건)': _i_unresolved(d),
                }
            _ka, _kb = _kpi_row(_fa), _kpi_row(_fb)
            _cmp_rows = []
            for k in _ka:
                a, b = _ka[k], _kb[k]
                d = a - b
                _cmp_rows.append({'지표': k, _la: a, _lb: b, '증감': f"{d:+,.1f}".rstrip('0').rstrip('.')})
            st.dataframe(pd.DataFrame(_cmp_rows), use_container_width=True, hide_index=True)
            # 설비 증감 TOP
            _ca = _fa['설비_KEY'].value_counts()
            _cb = _fb['설비_KEY'].value_counts()
            _allk = set(_ca.index) | set(_cb.index)
            _diff = sorted([(k, int(_ca.get(k, 0) - _cb.get(k, 0)), int(_ca.get(k, 0)), int(_cb.get(k, 0)))
                            for k in _allk], key=lambda x: x[1], reverse=True)
            _inc = [d for d in _diff if d[1] > 0][:8]
            if _inc:
                st.markdown("**📈 증가 설비 TOP**")
                st.dataframe(pd.DataFrame(_inc, columns=['설비', '증감', _la, _lb]),
                             use_container_width=True, hide_index=True)
            st.divider()

            # ⑨ 설비 드릴다운 ───────────────────────────
            st.markdown("### 🔎 설비 상세 드릴다운")
            _eqs = sorted(idf['설비_KEY'].dropna().unique().tolist())
            _sel_eq = st.selectbox("설비 선택", _eqs, key='t17_drill')
            if _sel_eq:
                _sub = idf[idf['설비_KEY'] == _sel_eq]
                _subv = _sub[_sub['소요시간'].notna()]
                dk1, dk2, dk3, dk4 = st.columns(4)
                dk1.metric("총 고장건수", f"{len(_sub):,}건")
                dk2.metric("평균 MTTR", f"{_subv['소요시간'].mean():.0f}분" if not _subv.empty else "N/A")
                dk3.metric("재발건수", f"{int(_sub['재발여부'].sum())}건" if '재발여부' in _sub.columns else "N/A")
                dk4.metric("미해결", f"{_i_unresolved(_sub)}건")
                dd1, dd2 = st.columns(2)
                with dd1:
                    st.markdown("**월별 추세**")
                    _sm = _sub.copy()
                    _sm['년월'] = _sm['발생일시'].dt.to_period('M').astype(str)
                    _smg = _sm.groupby('년월').size().reset_index(name='건수')
                    fig_sm = px.bar(_smg, x='년월', y='건수', color_discrete_sequence=['#1e3a5f'])
                    fig_sm.update_layout(height=260, margin=dict(t=10, b=20))
                    st.plotly_chart(fig_sm, use_container_width=True)
                with dd2:
                    st.markdown("**부위/현상 Top**")
                    if '고장부위' in _sub.columns and '현상' in _sub.columns:
                        _fk = (_sub['고장부위'].astype(str).replace('nan', '').str.strip() + ' / '
                               + _sub['현상'].astype(str).replace('nan', '').str.strip()).str.strip(' /').str.strip()
                        _fk = _fk[_fk != '']
                        if not _fk.empty:
                            _vc = _fk.value_counts().head(8)
                            _vcdf = pd.DataFrame({'부위/현상': _vc.index.tolist(),
                                                  '건수': _vc.values.tolist()})
                            st.dataframe(_vcdf, use_container_width=True, hide_index=True)
                st.markdown("**최근 조치 이력 (최신 8건)**")
                _hist_cols = [c for c in ['발생일시', '고장부위', '현상', '원인', '조치내역', '조치자', '소요시간']
                              if c in _sub.columns]
                _hist = _sub.sort_values('발생일시', ascending=False).head(8)[_hist_cols].copy()
                if '발생일시' in _hist.columns:
                    _hist['발생일시'] = _hist['발생일시'].dt.strftime('%Y-%m-%d %H:%M')
                st.dataframe(_hist, use_container_width=True, hide_index=True)


# ══════════════════════════════════════════════════════
# 백그라운드 워밍 — 현재 페이지 렌더 후, 남는 시간에 다른 탭 계산을 미리 데움
# (선택 탭 우선 표시 → 그 뒤 나머지 탭 무거운 계산을 스레드로 디스크 캐시에 채움)
# ══════════════════════════════════════════════════════
import threading as _threading

def _bg_warm_other_tabs(_dff):
...
if st.session_state.get('merged_df') is not None:
    _wf = apply_global_filter(st.session_state.merged_df)
    if _wf is not None and not _wf.empty and '발생일시' in _wf.columns:
        _wf = _wf[_wf['발생일시'].notna()]
        # 필터가 바뀔 때마다 1회만 백그라운드 워밍
        _wsig = (len(_wf), str(st.session_state.get('gf_label')))
        if st.session_state.get('_warmed_sig') != _wsig:
            st.session_state['_warmed_sig'] = _wsig
            try:
                _threading.Thread(target=_bg_warm_other_tabs,
                                  args=(_wf.copy(),), daemon=True).start()
            except Exception:
                pass
